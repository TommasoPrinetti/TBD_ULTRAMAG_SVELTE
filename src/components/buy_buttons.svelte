<script>
    ...
    onMount(() => {
        // PayPal SDK script is loaded only if the buying slider is open
        // This is because the PayPal button container div is not in the DOM until isOpen becomes true
        if (isOpen) {
            loadPaypalScript();
        }
    });
    ...
    function loadPaypalScript() {
        if (window.paypal) {
            // If the SDK is already defined, initialize the button
            initPayPalButton();
        } else {
    ...
    // PayPal button initialization function, unchanged
    ...
    <a class="button" id="BUY_ACTIVE" on:click={toggleSlider}>
        <p2> BUY NOW </p2>
    </a>
...
{#if isOpen}
    <div class="buying_slider">
        <!-- svelte-ignore a11y-click-events-have-key-events -->
        <div class="exit_slider" on:click={toggleSlider}>
